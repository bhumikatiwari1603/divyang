...
import { getAllMyOpenings} from "../../actions/openingActions";
  ...
  render() {
    ...
    if((this.props.openings).hasOwnProperty('result')){
      ...
      <tr key ={item.id} id={"opening-row-"+item.id}>
          ...
          <td><Link to={{pathname:"/editOpening",openingId:item.id}} ><button className="btn btn-sm btn-primary edit">Edit</button></Link>&nbsp;
          </td>
      </tr>
   ));
...
OpeningView.propTypes = {
  getAllMyOpenings: PropTypes.func.isRequired,
...
export default connect(mapStateToProps,{ getAllMyOpenings })(withRouter(OpeningView));